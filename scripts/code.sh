...
function code-wsl()
{
	...
	# in a wsl shell
	ELECTRON="$ROOT/.build/electron/Pdf Manager.exe"
	if [ -f "$ELECTRON"  ]; then
		local CWD=$(pwd)
		cd $ROOT
		export WSLENV=ELECTRON_RUN_AS_NODE/w:$WSLENV
		local WSL_EXT_ID="ms-vscode-remote.remote-wsl"
		local WSL_EXT_WLOC=$(ELECTRON_RUN_AS_NODE=1 "$ROOT/.build/electron/Pdf Manager.exe" "out/cli.js" --locate-extension $WSL_EXT_ID)
		cd $CWD
